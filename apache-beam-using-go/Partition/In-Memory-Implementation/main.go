// Licensed to the Apache Software Foundation (ASF) under one or more
// contributor license agreements.  See the NOTICE file distributed with
// this work for additional information regarding copyright ownership.
// The ASF licenses this file to You under the Apache License, Version 2.0
// (the "License"); you may not use this file except in compliance with
// the License.  You may obtain a copy of the License at
//
//    http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

// partition exemplifies using a cross-language partition transform from a test expansion service.
//
// Prerequisites to run wordcount:
// –> [Required] Job needs to be submitted to a portable runner (--runner=universal)
// –> [Required] Endpoint of job service needs to be passed (--endpoint=<ip:port>)
// –> [Required] Endpoint of expansion service needs to be passed (--expansion_addr=<ip:port>)
// –> [Optional] Environment type can be LOOPBACK. Defaults to DOCKER. (--environment_type=LOOPBACK|DOCKER)
package main

import (
	"context"
	"fmt"
	"log"

	"github.com/apache/beam/sdks/v2/go/pkg/beam"
	"github.com/apache/beam/sdks/v2/go/pkg/beam/runners/direct"
)

func main() {

	pipeline, s := beam.NewPipelineWithRoot()

	// Create a PCollection with sample data
	data := []int{1, 2, 3, 4, 5, 6}
	input := beam.CreateList(s, data)

	// Partition the data into two partitions based on even and odd numbers
	partitioned := beam.ParDo(s, partitionFn, input)

	// Collect the partitions into separate slices
	var evenPartition []int
	var oddPartition []int

	beam.ParDo(s, collectFn(&evenPartition), partitioned)
	beam.ParDo(s, collectFn(&oddPartition), partitioned)

	// Print the partitions
	evenStr := fmt.Sprintf("Even Partition: %v", evenPartition)
	oddStr := fmt.Sprintf("Odd Partition: %v", oddPartition)

	log.Println(evenStr)
	log.Println(oddStr)

	if presponse, err := direct.Execute(context.Background(), pipeline); err != nil {
		fmt.Println("", presponse)
		log.Fatalf("Pipeline execution failed: %v", err)
	}

}

func partitionFn(num int) (int, int) {
	if num%2 == 0 {
		return 0, num // Partition 0 for even numbers
	}
	return 1, num // Partition 1 for odd numbers
}

func collectFn(slice *[]int) func(int) {
	return func(num int) {
		*slice = append(*slice, num)
	}
}

/*
package main

import (
	"context"
	"flag"
	"fmt"
	"github.com/apache/beam/sdks/v2/go/pkg/beam/runners/direct"
	"log"

	"github.com/apache/beam/sdks/v2/go/examples/xlang"
	"github.com/apache/beam/sdks/v2/go/pkg/beam"
	"github.com/apache/beam/sdks/v2/go/pkg/beam/testing/passert"
	// Imports to enable correct filesystem access and runner setup in LOOPBACK mode
	_ "github.com/apache/beam/sdks/v2/go/pkg/beam/io/filesystem/gcs"
	_ "github.com/apache/beam/sdks/v2/go/pkg/beam/io/filesystem/local"
)

var (
	expansionAddr = flag.String("expansion_addr", "output/output.txt", "Address of Expansion Service")
)

// formatFn is a DoFn that formats a word and its count as a string.
func formatFn(c int64) string {
	return fmt.Sprintf("%v", c)
}

func init() {
	beam.RegisterFunction(formatFn)
}

func main() {
	flag.Parse()
	beam.Init()

	if *expansionAddr == "" {
		log.Fatal("No expansion address provided")
	}

	p := beam.NewPipeline()
	s := p.Root()

	col := beam.CreateList(s, []int64{1, 2, 3, 4, 5, 6})

	// Using the cross-language transform
	out0, out1 := xlang.Partition(s, *expansionAddr, col)
	formatted0 := beam.ParDo(s, formatFn, out0)
	formatted1 := beam.ParDo(s, formatFn, out1)

	passert.Equals(s, formatted0, "2", "4", "6")
	passert.Equals(s, formatted1, "1", "3", "5")

	direct.Execute(context.Background(), p)
	//if err := beamx.Run(context.Background(), p); err != nil {
	//	log.Fatalf("Failed to execute job: %v", err)
	//}
}
*/
/*package main

import (
	"context"
	"log"
	"strings"

	"github.com/apache/beam/sdks/v2/go/pkg/beam"
	"github.com/apache/beam/sdks/v2/go/pkg/beam/io/textio"
	"github.com/apache/beam/sdks/v2/go/pkg/beam/x/beamx"
)

func main() {
	ctx := context.Background()
	pipeline, s := beam.NewPipelineWithRoot()

	// Read input data from a text file
	lines := textio.Read(s, "input.txt")

	// Split lines into individual words
	words := beam.ParDo(s, extractWordsFn, lines)

	// Partition words based on their lengths
	partitioned := beam.Partition(s, 2, lengthPartitionFn, words)

	// Apply a processing function to each partition
	resultPartitions := beam.ParDo(s, processPartitionFn, partitioned)

	// Flatten the result partitions back into a single PCollection
	results := beam.Flatten(s, resultPartitions)

	// Write the results to an output file
	textio.Write(s, "output.txt", results)

	if err := beamx.Run(ctx, pipeline); err != nil {
		log.Fatalf("Pipeline execution failed: %v", err)
	}
}

func extractWordsFn(line string, emit func(string)) {
	words := strings.Fields(line)
	for _, word := range words {
		emit(word)
	}
}

func lengthPartitionFn(word string, numPartitions int) int {
	return len(word) % numPartitions
}

func processPartitionFn(_ int, words func(*string) bool) string {
	var result string
	var wordPtr *string
	for words(wordPtr) {
		result += *wordPtr + " "
	}
	return result
}
*/
