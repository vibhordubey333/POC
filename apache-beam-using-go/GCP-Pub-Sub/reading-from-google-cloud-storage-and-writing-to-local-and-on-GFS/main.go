...
var (
	...
	input = flag.String("input", "gs://data-flow-bucket-vd/abc.txt", "File(s) to read.")

	// Set this required option to specify where to write the output.
	output  = flag.String("output", "output.txt", "Output file (required).")
	output2 = flag.String("output:gcs", "gs://data-flow-bucket-vd/output.txt", "Google Cloud Storage Path")
)
...
func main() {
	...
	//Writing to text file locally
	textio.Write(s, *output, formatted)
	//Writing to text file present on GCS. If file doesn't exist it will get created like local storage.
	textio.Write(s, *output2, formatted)
